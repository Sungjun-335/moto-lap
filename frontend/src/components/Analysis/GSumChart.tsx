...
import {
    LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ReferenceArea, ReferenceLine, ReferenceDot
} from 'recharts';
...
const GSumChart: React.FC<GSumChartProps> = ({
    ...
    return (
        <div style={{ width: '100%', height: height }} className="bg-zinc-900 rounded-lg border border-zinc-800 p-2 flex flex-col" onMouseUp={onMouseUp}>
            <h3 className="text-xs text-zinc-400 font-medium mb-1 ml-2">{t.charts.combinedG} <span className="text-zinc-500">{t.charts.gSumSuffix}</span></h3>
            <div className="flex-1 w-full min-h-0">
                <ResponsiveContainer>
                    ...
                    >
                        ...
                        <Line type="monotone" dataKey="anaGSum" stroke="#ef4444" dot={false} strokeWidth={2} name="G-Sum (Ana)" />
                        <Line type="monotone" dataKey="refGSum" stroke="#3b82f6" dot={false} strokeWidth={2} name="G-Sum (Ref)" />
...
export default React.memo(GSumChart, (prev, next) => {
